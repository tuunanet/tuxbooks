...
/**
 * PDFium's `FPDF_COLORSCHEME` (ADR 0002) recolors page content by category:
 * path fill and stroke, text fill and stroke, as 32-bit `0xAARRGGBB` values.
 * Images are not a category, so photographs and scanned rasters keep their
 * pixels. This is the coarse successor to the MuPDF object-recolor device;
 * the palette (`SmartPalette`) is mapped onto it by `colorSchemeFromPalette`.
 */

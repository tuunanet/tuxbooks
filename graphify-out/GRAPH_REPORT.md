# Graph Report - tuxbooks  (2026-09-08)

## Corpus Check
- 306 files · ~527,564 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 9186 nodes · 20712 edges · 334 communities (222 shown, 96 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 342 edges (avg confidence: 0.83)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9217a9d0`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- pdf.worker.mjs
- getStringOption
- ConfigNamespace
- Dict
- .add
- cn
- StringObject
- .get
- AnnotationEditorUIManager
- AnnotationEditor
- DrawingEditor
- PdfDocumentView.tsx
- pdf.mjs
- warn
- .push
- AppShell.tsx
- .success
- PdfReader.tsx
- .getOperatorList
- ContentObject
- FormatError
- epub/epubEngine.ts
- .add
- makeBrotliDecode
- .checkAndRepair
- lib/bridge.ts
- IccColorSpace
- services/annotations.rs
- AnnotationEditorLayer
- .convert
- XhtmlObject
- PDFPageProxy
- XFAObject
- Comment
- StampEditor
- BasePDFStreamReader
- services/metadata.rs
- ExclGroup
- EventEmitter
- .setupMessageHandler
- CanvasGraphics
- LibraryView.tsx
- intersectionObserver.ts
- book_importer.rs
- .render
- CanvasDependencyTracker
- FontInfo
- .remove
- AnnotationStorage
- calculateSHA512
- .get
- pdf/parser.rs
- ReadiumEpubHandle
- repository/books.rs
- session.rs
- AppError
- .getTextContent
- unreachable
- foliate-js/package.json
- EpubError
- IntegerObject
- make-epub-fixtures.py
- rpc.rs
- readiumEngine.ts
- PDFDocumentProxy
- Option01
- Annotation
- find
- JBig2
- setup/fixtures.ts
- frontend/package.json
- epubcfi.js
- shadow
- WorkerTransport
- PDFDataRangeTransport
- .compile
- View
- helpers.ts
- dict.js
- CanvasNestedDependencyTracker
- .compileGlyph
- PopupElement
- .getToken
- library_watcher.rs
- index.ts
- ReaderNavigation.tsx
- BaseCanvasFactory
- Glyph
- Reconciler
- package.json
- opds.js
- e2e/package.json
- repository/reading_progress.rs
- epub.js
- $
- DOMFilterFactory
- library_scanner.rs
- bundle
- mobi.js
- .put
- info
- epub/metadata.rs
- tts.js
- assert
- FontFaceObject
- SimpleDOMNode
- view.js
- KF8
- MessageHandler
- getPdfManager
- What You Must Do When Invoked
- repository/collections.rs
- FontInfo
- Documentation
- XMLParserBase
- vendor/zip.js
- search.rs
- devDependencies
- Paginator
- PagesMapper
- JpegImage
- ColorSpace
- compilerOptions
- .bind
- 4K scrolling performance — implementation plan
- components.json
- FixedLayout
- ColorPicker
- InkDrawOutliner
- warn
- LabCS
- dependencies
- progressMigration.ts
- .wrap
- EpubReader.test.tsx
- .constructor
- fb2.js
- .startDrawing
- ArithmeticDecoder
- .fill
- getTextRegionHuffmanTables
- commands/books.rs
- MOBI
- OpenJPEG
- Util
- ChunkedStream
- Util
- SignatureExtractor
- InkDrawOutline
- .getEntriesGenerator
- CFFCompiler
- MediaOverlay
- FreeDrawOutliner
- XmlObject
- .readBit
- .constructor
- NewBook
- Stages
- 4K scrolling performance — investigation handover
- MOBI6
- paginator.js
- XFAFactory
- decodeScan
- artwork_cache.rs
- Rendering
- Overlayer
- getFilenameFromContentDispositionHeader
- ColorConverters
- .exec
- Outline
- .process
- Binder
- Builder
- ColorConverters
- BasePDFStream
- services/reader.rs
- Tables
- EPUB
- Loader
- pdfRenderPolicy.test.ts
- bench-reader.e2e.ts
- compilerOptions
- View
- CanvasExtraState
- HighlightOutline
- CFFDict
- AGENTS.md
- Testing
- metadata.e2e.ts
- versions.ts
- .constructor
- footnotes.js
- NullOptimizer
- Xe
- pdfOutline.ts
- make-fixture.py
- Coding standards
- .#goTo
- SectionProgress
- IdentityCMap
- CalRGBCS
- XhtmlNamespace
- y
- commands/reader.rs
- Reader rendering (Readium TS Toolkit)
- TuxbooksApi
- BasePDFStreamReader
- Datasets
- TextMeasure
- XFAAttribute
- extended_epub.rs
- Architecture
- Reader performance — bench findings and handover
- watchdog.mjs
- pdf.js
- CommandManager
- ue
- graphify reference: extra exports and benchmark
- epub_corpus.rs
- Build and dev environment
- Electron migration plan
- Performance budgets and metrics
- fetch-chromedriver.mjs
- scripts
- getStruct
- .#turnPage
- MetadataParser
- JpegStream
- History
- TuxBooks
- ROADMAP.md
- Development Principles
- Milestone 6 — Bookmarks, Highlights, and Notes
- Milestone 4 — Covers and Artwork Pipeline
- Scope
- Milestone 2 — PDF Navigation
- fetch-epub-extended.py
- EPUB layer
- Release and distribution
- Webview frame clock and dpr fiction — findings
- search.js
- FreeDrawOutline
- books
- DeviceRgbaCS
- SingleIntersector
- Milestone 10 — Library and Reader UX Polish
- Milestone 7 — Metadata and Library Curation
- Milestone 3 — Filesystem Watcher and Library Reconciliation
- BookMetadata
- realistic_library_imports_every_book_file
- epub-reader.e2e.ts
- progress-migration.e2e.ts
- Subform
- AnnotationBorderStyle
- ve
- graphify reference: query, path, explain
- Milestone 8 — Unified Reader Model
- Milestone 9 — Reader Reliability and Performance Hardening
- Milestone 11 — Release and Distribution
- Milestone 5 — Search
- default.json
- ReadingProgress
- foliate-js.d.ts
- Encryption
- .init
- quote-image.js
- epubcfi-tests.js
- FoliateView
- .parseXml
- signature_Signature
- Stylesheet
- CursorAutohider
- .prettierrc.json
- Button
- Coverage gate
- Confirming comment for WebKit bug 315997
- .render
- GlobalColorSpaceCache
- SimpleXMLParser
- PostScriptParser
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- coverage-gate.mjs
- PostScriptToken
- preload.ts
- Qe
- opencode.json
- graphify.js
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- check-deb.sh
- dev-env.sh
- main.rs
- Area
- brand/README.md
- extraction-spec.md
- fetch-pdfium.sh
- install-actionlint.sh
- run-parallel.sh
- img/README.md
- domain/library.rs
- tuxbooks
- library_locations.rs
- Br
- LocalPdfManager
- xdp_Xdp
- Body
- Html
- I
- Color
- P
- 0002_books_fts.sql
- 0005_library_sync.sql
- 0006_books_fts_extended.sql
- $root
- Span
- Sub

## God Nodes (most connected - your core abstractions)
1. `XFAObject` - 217 edges
2. `warn()` - 179 edges
3. `AnnotationEditor` - 171 edges
4. `AppError` - 149 edges
5. `ConfigNamespace` - 141 edges
6. `AnnotationEditorUIManager` - 138 edges
7. `TemplateNamespace` - 115 edges
8. `CanvasGraphics` - 102 edges
9. `cn()` - 101 edges
10. `shadow()` - 100 edges

## Surprising Connections (you probably didn't know these)
- `openReadyEpub()` --calls--> `openInReader()`  [EXTRACTED]
  e2e/specs/epub-reader.e2e.ts → e2e/specs/helpers.ts
- `GlobalSearchShortcut()` --calls--> `useShortcut()`  [EXTRACTED]
  frontend/src/components/layout/AppShell.tsx → frontend/src/lib/shortcuts.ts
- `SidebarProps` --references--> `LibrarySection`  [EXTRACTED]
  frontend/src/components/layout/Sidebar.tsx → frontend/src/state/appState.ts
- `ItemButton()` --calls--> `cn()`  [EXTRACTED]
  frontend/src/components/layout/Sidebar.tsx → frontend/src/lib/utils.ts
- `TocRow` --references--> `EpubTocItem`  [EXTRACTED]
  frontend/src/components/reader/ReaderNavigation.tsx → frontend/src/lib/epub/epubEngine.ts

## Import Cycles
- 2-file cycle: `src-tauri/src/epub/metadata.rs -> src-tauri/src/epub/mod.rs -> src-tauri/src/epub/metadata.rs`
- 2-file cycle: `src-tauri/src/epub/mod.rs -> src-tauri/src/epub/session.rs -> src-tauri/src/epub/mod.rs`

## Communities (334 total, 96 thin omitted)

### Community 0 - "pdf.worker.mjs"
Cohesion: 0.01
Nodes (316): $acceptWhitespace, $addHTML, AnnotationActionEventType, AnnotationBorderStyleType, AnnotationEditorParamsType, AnnotationEditorType, AnnotationFieldFlag, AnnotationFlag (+308 more)

### Community 1 - "getStringOption"
Cohesion: 0.02
Nodes (39): Agent, CalendarSymbols, CurrencySymbol, CurrencySymbols, $data, DatePattern, DatePatterns, Day (+31 more)

### Community 2 - "ConfigNamespace"
Cohesion: 0.01
Nodes (79): Acrobat, Acrobat7, ADBE_JSConsole, ADBE_JSDebugger, _attributes, AutoSave, BatchOutput, Cache (+71 more)

### Community 3 - "Dict"
Cohesion: 0.04
Nodes (40): ButtonWidgetAnnotation, CaretAnnotation, ChoiceWidgetAnnotation, CircleAnnotation, Dict, ErrorFont, escapeString(), FakeUnicodeFont (+32 more)

### Community 4 - ".add"
Cohesion: 0.03
Nodes (30): AnnotationElement, AnnotationElementFactory, bindEvents(), CaretAnnotationElement, CheckboxWidgetAnnotationElement, ChoiceWidgetAnnotationElement, CircleAnnotationElement, FileAttachmentAnnotationElement (+22 more)

### Community 5 - "cn"
Cohesion: 0.03
Nodes (127): BookCard(), BookCardProps, InteractiveBookProps, progressPercentOf(), BookContextMenu(), BookContextMenuProps, BookCover(), BookCoverProps (+119 more)

### Community 6 - "StringObject"
Cohesion: 0.02
Nodes (43): Amd, AppearanceFilter, Base, Certificate, config_Picture, connection_set_Uri, ConnectionSet, ConnectionSetNamespace (+35 more)

### Community 7 - ".get"
Cohesion: 0.03
Nodes (37): Catalog, appendIfJavaScriptDict(), addPageDict(), addPageError(), pagesBeforeRef(), parseNestedOrder(), parseOnOff(), parseOrder() (+29 more)

### Community 10 - "DrawingEditor"
Cohesion: 0.03
Nodes (8): DrawingEditor, DrawingOptions, DrawLayer, DrawnSignatureOptions, HighlightEditor, InkDrawingOptions, SignatureEditor, SignatureOptions

### Community 11 - "PdfDocumentView.tsx"
Cohesion: 0.05
Nodes (54): useFitWidthScale(), GeometryState, PdfGeometry, PdfAnchorInfo, PdfScrollTrackingOptions, READING_ANCHOR_RATIO, usePdfScrollTracking(), PdfVirtualization (+46 more)

### Community 12 - "pdf.mjs"
Cohesion: 0.02
Nodes (85): AnnotationActionEventType, AnnotationBorderStyleType, AnnotationEditorParamsType, AnnotationEditorType, AnnotationFieldFlag, AnnotationFlag, AnnotationMode, AnnotationReplyType (+77 more)

### Community 13 - "warn"
Cohesion: 0.05
Nodes (15): AnnotationFactory, DocumentData, fetchBinaryData(), getFontNameToFileMap, getNewAnnotationsMap(), JpxImage, makeArr(), normalizeBlendMode() (+7 more)

### Community 14 - ".push"
Cohesion: 0.05
Nodes (38): addChildren(), buildHuffmanTable(), compileType3Glyph(), computeIDs(), computeMD5(), createDataNode(), createDefaultAppearance(), encodeToXmlString() (+30 more)

### Community 15 - "AppShell.tsx"
Cohesion: 0.05
Nodes (75): App(), BookDetail(), CollectionDialog(), AppShell(), AppShellProps, GlobalSearchShortcut(), Shell(), ItemButton() (+67 more)

### Community 16 - ".success"
Cohesion: 0.05
Nodes (36): applyAssist(), ariaLabel(), Caption, CheckButton, checkDimensions(), ChoiceList, computeBbox(), Corner (+28 more)

### Community 17 - "PdfReader.tsx"
Cohesion: 0.04
Nodes (96): annotationRects(), byKind(), DEFAULT_HIGHLIGHT_COLOR, HIGHLIGHT_COLORS, HighlightColor, highlightCssColor(), isBookmarkAtCfi(), isBookmarkAtPage() (+88 more)

### Community 18 - ".getOperatorList"
Cohesion: 0.04
Nodes (28): addCachedImageOps(), BaseLocalCache, BaseShading, CMapFactory, DummyShading, EvalState, getEncoding(), getSymbolsFonts (+20 more)

### Community 19 - "ContentObject"
Cohesion: 0.02
Nodes (24): AlwaysEmbed, BehaviorOverride, BooleanElement, ContentObject, DateElement, DateTime, DateTimeSymbols, Decimal (+16 more)

### Community 20 - "FormatError"
Cohesion: 0.06
Nodes (29): BaseException, CipherTransform, Cmd, createBuiltInCMap(), expectInt(), expectString(), extendCMap(), FormatError (+21 more)

### Community 21 - "epub/epubEngine.ts"
Cohesion: 0.05
Nodes (24): EpubDocumentSnapshot, EpubDocumentState, EpubDocumentStatus, asString(), EPUB_FONT_FAMILIES, EPUB_MIME_TYPE, EPUB_SCROLLED_SURFACE_MAX_PX, EpubAppearance (+16 more)

### Community 22 - ".add"
Cohesion: 0.08
Nodes (19): AppearanceStreamEvaluator, CFFFDSelect, CFFParser, parseFloatOperand(), parseOperand(), readSupplement(), compileCharString(), bezierCurveTo() (+11 more)

### Community 23 - "makeBrotliDecode"
Cohesion: 0.06
Nodes (66): GlobalImageCache, makeBrotliDecode(), attachDictionaryChunk(), buildHuffmanTable(), bytesToNibbles(), calculateDistanceAlphabetLimit(), calculateDistanceAlphabetSize(), calculateDistanceLut() (+58 more)

### Community 24 - ".checkAndRepair"
Cohesion: 0.02
Nodes (58): adjustMapping(), Ascii85Stream, AsciiHexStream, BrotliDecode, BrotliStream, bytesToString(), CCITTFaxStream, DecodeStream (+50 more)

### Community 25 - "lib/bridge.ts"
Cohesion: 0.06
Nodes (72): DropZoneOverlay(), EmptyLibraryState(), ImportStatus(), LibraryHeader(), PROGRESS_SAVE_DEBOUNCE_MS, ReaderProgressOptions, useReaderProgress(), AnnotationSnapshot (+64 more)

### Community 26 - "IccColorSpace"
Cohesion: 0.12
Nodes (4): CmykICCBasedCS, ColorSpaceUtils, fetchSync(), IccColorSpace

### Community 27 - "services/annotations.rs"
Cohesion: 0.05
Nodes (82): AnnotationRect, AnnotationDto, AnnotationInput, AnnotationPatchInput, AnnotationRect, create_annotation(), delete_annotation(), list_annotations() (+74 more)

### Community 29 - ".convert"
Cohesion: 0.03
Nodes (47): adjustTrueTypeToUnicode(), adjustType1ToUnicode(), adjustWidths(), amendFallbackToUnicode(), applyStandardFontGlyphMap(), buildToFontChar(), CFF, CFFFont (+39 more)

### Community 30 - "XhtmlObject"
Cohesion: 0.17
Nodes (4): B, checkStyle(), Ul, XhtmlObject

### Community 31 - "PDFPageProxy"
Cohesion: 0.04
Nodes (11): AbortException, getReadableStream(), getXfaPageViewport(), InternalRenderTask, makeObj(), PageViewport, PDFPageProxy, operatorListChanged() (+3 more)

### Community 32 - "XFAObject"
Cohesion: 0.01
Nodes (71): Arc, Assist, Barcode, Bind, BindItems, Bookend, Border, Break (+63 more)

### Community 33 - "Comment"
Cohesion: 0.04
Nodes (5): Comment, EditorToolbar, FloatingToolbar, stopEvent(), TouchManager

### Community 34 - "StampEditor"
Cohesion: 0.05
Nodes (4): AltText, ImageManager, OutputScale, StampEditor

### Community 35 - "BasePDFStreamReader"
Cohesion: 0.04
Nodes (19): BasePDFStreamRangeReader, BasePDFStreamReader, createResponseError(), ensureResponseOrigin(), ensureResponseStatus(), extractFilenameFromHeader(), fetch_stream_getArrayBuffer(), fetchUrl() (+11 more)

### Community 36 - "services/metadata.rs"
Cohesion: 0.14
Nodes (51): apply_source_metadata(), blank_title_is_rejected(), clean_list(), clean_optional(), clean_required(), clear_book_cover_override(), clearing_a_field_explicitly_overrides_it_to_empty(), cover_override_wins_and_survives_reimport() (+43 more)

### Community 37 - "ExclGroup"
Cohesion: 0.11
Nodes (4): ExclGroup, Image, $setValue, Value

### Community 38 - "EventEmitter"
Cohesion: 0.11
Nodes (38): ImportReport, import_paths(), reconnect_book(), AppState, Book, Result, String, Vec (+30 more)

### Community 39 - ".setupMessageHandler"
Cohesion: 0.05
Nodes (17): applyOpacity(), BaseException, dataObj(), endRequests(), FontPathInfo, FormatError, InvalidPDFException, MathClamp() (+9 more)

### Community 40 - "CanvasGraphics"
Cohesion: 0.05
Nodes (11): CanvasGraphics, copyCtxState(), drawImageAtIntegerCoords(), getCurrentTransform(), getCurrentTransformInverse(), getImageSmoothingEnabled(), mirrorContextOperations(), putBinaryImageData() (+3 more)

### Community 41 - "LibraryView.tsx"
Cohesion: 0.12
Nodes (25): BookListItem(), EmptyCollectionState(), LibraryHeaderProps, columnCount(), LibraryView(), LibraryViewProps, NoSearchResultsState(), NoSearchResultsStateProps (+17 more)

### Community 42 - "intersectionObserver.ts"
Cohesion: 0.13
Nodes (9): fireIntersection(), installMockIntersectionObserver(), intersectionObservers(), MockIntersectionObserver, resetIntersectionObservers(), firePreload(), fireVisible(), firePreload() (+1 more)

### Community 43 - "book_importer.rs"
Cohesion: 0.10
Nodes (56): ProgressUpdate, init_pool(), init_pool_is_deterministic_and_idempotent(), Path, Result, SqlitePool, run_migrations(), schema_contains_all_core_tables() (+48 more)

### Community 44 - ".render"
Cohesion: 0.06
Nodes (7): AnnotationLayer, BaseSVGFactory, DOMSVGFactory, EditorAnnotationElement, InkAnnotationElement, makeArr(), setLayerDimensions()

### Community 45 - "CanvasDependencyTracker"
Cohesion: 0.05
Nodes (4): BBoxReader, CanvasDependencyTracker, ensureDebugMetadata(), expandBBox()

### Community 46 - "FontInfo"
Cohesion: 0.07
Nodes (4): assert(), CssFontInfo, FontInfo, SystemFontInfo

### Community 48 - "AnnotationStorage"
Cohesion: 0.05
Nodes (5): AnnotationStorage, BasicColorPicker, InkEditor, MurmurHash3_64, PrintAnnotationStorage

### Community 49 - "calculateSHA512"
Cohesion: 0.07
Nodes (20): AES128Cipher, AES256Cipher, AESBaseCipher, ARCFourCipher, calculateMD5(), calculateSHA384(), calculateSHA512(), ch() (+12 more)

### Community 50 - ".get"
Cohesion: 0.07
Nodes (6): ColorManager, FakeEditor, getColorValues(), getRGB(), LinkAnnotationElement, makeMap()

### Community 51 - "pdf/parser.rs"
Cohesion: 0.10
Nodes (39): Document, Object, PdfError, Error, String, build_pdf(), decode_pdf_string(), empty_title_field_falls_back_to_file_name() (+31 more)

### Community 52 - "ReadiumEpubHandle"
Cohesion: 0.07
Nodes (6): ProgressSource, asString(), clamp01(), highlightTint(), ReadiumEpubHandle, serializeLocator()

### Community 53 - "repository/books.rs"
Cohesion: 0.17
Nodes (38): count_books(), delete_book(), delete_cascades_to_reading_progress(), duplicate_path_violates_unique_constraint(), empty_title_is_rejected_by_check_constraint(), find_books_with_size(), find_id_by_path(), get_book() (+30 more)

### Community 54 - "session.rs"
Cohesion: 0.12
Nodes (39): local_name(), books_without_toc_documents_read_without_a_toc(), build_manifest_json(), build_positions_json(), build_session(), builds_manifest_with_reading_order_and_toc(), builds_positions_across_spine(), clamp01() (+31 more)

### Community 55 - "AppError"
Cohesion: 0.13
Nodes (38): MigrateError, add_book_to_collection(), create_collection(), delete_collection(), list_collections(), remove_book_from_collection(), AppState, CollectionSummary (+30 more)

### Community 56 - ".getTextContent"
Cohesion: 0.09
Nodes (19): DefaultAppearanceEvaluator, EvaluatorPreprocessor, Intersector, normalizeUnicode(), addFakeSpaces(), appendEOL(), applyInverseRotation(), buildTextContentItem() (+11 more)

### Community 57 - "unreachable"
Cohesion: 0.07
Nodes (4): BasePdfManager, BaseStream, Pattern, unreachable()

### Community 58 - "foliate-js/package.json"
Cohesion: 0.05
Nodes (36): author, bugs, url, description, devDependencies, @eslint/js, fflate, fs-extra (+28 more)

### Community 59 - "EpubError"
Cohesion: 0.13
Nodes (38): empty_zip_reports_missing_mimetype(), EpubError, mimetype_not_first_entry_reports_missing_mimetype(), missing_container_xml_reports_missing_container(), not_a_zip_reports_zip_error(), Error, String, wrong_mimetype_reports_invalid_mimetype() (+30 more)

### Community 60 - "IntegerObject"
Cohesion: 0.05
Nodes (13): AdjustData, AdobeExtensionLevel, CompressObjectStream, Copies, CurrentPage, IntegerObject, Level, MsgId (+5 more)

### Community 61 - "make-epub-fixtures.py"
Cohesion: 0.08
Nodes (30): build_all(), build_valid(), chapter_paragraph(), check(), emit_manifest(), main(), make(), _minimal_entries() (+22 more)

### Community 62 - "rpc.rs"
Cohesion: 0.09
Nodes (33): BookBytesArgs, BookIdArgs, BookResourceArgs, CollectionIdArgs, CollectionMemberArgs, CoverArgs, delete_collection_takes_the_bridge_wire_param(), dispatch() (+25 more)

### Community 63 - "readiumEngine.ts"
Cohesion: 0.07
Nodes (28): SerializedLocator, buildPositions(), EPUB_FONT_FAMILIES, EPUB_MIME_TYPE, EPUB_SCROLLED_SURFACE_MAX_PX, EpubAppearance, EpubFlow, EpubFontFamily (+20 more)

### Community 65 - "Option01"
Cohesion: 0.03
Nodes (20): AddSilentPrint, AddViewerPreferences, Change, CompressLogicalStructure, config_Encrypt, ContentCopy, DocumentAssembly, Embed (+12 more)

### Community 67 - "find"
Cohesion: 0.07
Nodes (13): find(), FontFinder, fonts_getMetrics(), FontSelector, getCurrentPara(), makeObj(), PageSet, selectFont() (+5 more)

### Community 68 - "JBig2"
Cohesion: 0.13
Nodes (11): JBig2(), postRun(), preRun(), run(), doRun(), abort(), initRuntime(), postRun() (+3 more)

### Community 69 - "setup/fixtures.ts"
Cohesion: 0.09
Nodes (29): armTeardownWatchdog(), artifactsDir, configDir, databasePath, killStaleProcesses(), prepareEnvironment(), pruneOldArtifacts(), runId (+21 more)

### Community 70 - "frontend/package.json"
Cohesion: 0.06
Nodes (32): @eslint/js, globals, pdfjs-dist, @types/node, typescript, license, name, private (+24 more)

### Community 71 - "epubcfi.js"
Cohesion: 0.13
Nodes (33): buildRange(), collapse(), compare(), concatArrays(), escapeCFI(), fake, findIndices(), findTokens() (+25 more)

### Community 72 - "shadow"
Cohesion: 0.16
Nodes (3): ColorScheme, FeatureTest, shadow()

### Community 73 - "WorkerTransport"
Cohesion: 0.07
Nodes (3): isRefProxy(), Metadata, WorkerTransport

### Community 74 - "PDFDataRangeTransport"
Cohesion: 0.07
Nodes (9): BasePDFStream, createHeaders(), deprecated(), getArrayBuffer(), PDFDataRangeTransport, PDFDataTransportStream, PDFFetchStream, PDFNetworkStream (+1 more)

### Community 75 - ".compile"
Cohesion: 0.09
Nodes (13): AstArgument, AstBinaryOperation, AstLiteral, AstMin, AstNode, AstVariable, AstVariableDefinition, buildAddOperation() (+5 more)

### Community 77 - "helpers.ts"
Cohesion: 0.17
Nodes (27): bitmapCacheUsage(), canvasIsNonBlank(), clickRetryingStale(), clickUntilEffect(), closeReaderNavigation(), currentPageNumber(), epubHostCount(), epubSectionTotal() (+19 more)

### Community 78 - "dict.js"
Cohesion: 0.09
Nodes (11): concatTypedArray(), decode, decodeBase64Number(), decoder, DictdDict, DictdIndex, DictZip, Index (+3 more)

### Community 80 - ".compileGlyph"
Cohesion: 0.10
Nodes (10): Commands, CompiledFont, FontRendererFactory, getSubroutineBias(), lookupCmap(), parseCff(), parseGlyfTable(), buildPath() (+2 more)

### Community 83 - "library_watcher.rs"
Cohesion: 0.06
Nodes (68): Duration, EventKind, FnOnce, Formatter, Debug, LibraryChange, Mutex, RecommendedWatcher (+60 more)

### Community 84 - "index.ts"
Cohesion: 0.13
Nodes (21): bookMime(), COVER_MIME_BY_EXTENSION, coverMime(), coversDir(), createWindow(), DEFAULT_STATE, loadWindowState(), parseRange() (+13 more)

### Community 85 - "ReaderNavigation.tsx"
Cohesion: 0.12
Nodes (20): COLOR_ORDER, ReaderAnnotationList(), ReaderAnnotationListProps, flattenOutline(), flattenToc(), OutlineRow, ReaderNavigation(), ReaderNavTab (+12 more)

### Community 86 - "BaseCanvasFactory"
Cohesion: 0.20
Nodes (3): BaseCanvasFactory, DOMCanvasFactory, NodeCanvasFactory

### Community 87 - "Glyph"
Cohesion: 0.07
Nodes (7): CompositeGlyph, Contour, FontPathInfo, GlyfTable, Glyph, GlyphHeader, SimpleGlyph

### Community 88 - "Reconciler"
Cohesion: 0.18
Nodes (18): Handle, OnChange, file_stats(), is_book_extension(), is_book_path(), LibraryChange, PathChange, Reconciler (+10 more)

### Community 89 - "package.json"
Cohesion: 0.06
Nodes (29): description, devDependencies, electron, esbuild, prettier, @types/node, typescript, electron (+21 more)

### Community 90 - "opds.js"
Cohesion: 0.12
Nodes (27): RFC-6570, RFC-7231, FACET_GROUP, filterNS(), getContent(), getFeed(), getIndirectAcquisition(), getLink() (+19 more)

### Community 91 - "e2e/package.json"
Cohesion: 0.07
Nodes (29): devDependencies, electron, tslib, @types/node, @wdio/cli, @wdio/electron-service, @wdio/local-runner, @wdio/mocha-framework (+21 more)

### Community 92 - "repository/reading_progress.rs"
Cohesion: 0.14
Nodes (29): get_reading_progress(), mark_book_finished(), ProgressInput, ProgressUpdate, AppState, From, Option, Result (+21 more)

### Community 93 - "epub.js"
Cohesion: 0.11
Nodes (26): camel(), deobfuscate(), deobfuscators(), getAttributes(), getDisplayOptions(), getElementText(), getIdentifier(), getMetadata() (+18 more)

### Community 94 - "$"
Cohesion: 0.12
Nodes (19): $, dropTarget, formatContributor(), formatLanguageMap(), formatOneContributor(), getCSS(), listFormat, open() (+11 more)

### Community 95 - "DOMFilterFactory"
Cohesion: 0.14
Nodes (3): BaseFilterFactory, DOMFilterFactory, NodeFilterFactory

### Community 96 - "library_scanner.rs"
Cohesion: 0.20
Nodes (24): BookParseError, directories_named_like_epub_files_are_ignored(), empty_directory_yields_no_entries(), entries_are_sorted_by_path(), extension_match_is_case_insensitive(), file_as_root_is_not_a_directory(), finds_epub_in_nested_directories_and_ignores_other_files(), has_book_extension() (+16 more)

### Community 97 - "bundle"
Cohesion: 0.07
Nodes (28): app, security, windows, withGlobalTauri, enable, scope, build, beforeBuildCommand (+20 more)

### Community 98 - "mobi.js"
Cohesion: 0.09
Nodes (25): CDIC_HEADER, countBitsSet(), countUnsetEnd(), decoder, EXTH_HEADER, EXTH_RECORD_TYPE, FDST_HEADER, FONT_HEADER (+17 more)

### Community 100 - "info"
Cohesion: 0.13
Nodes (7): CCITTFaxDecoder, info(), LocalFunctionCache, PDFFunction, PDFFunctionFactory, PostScriptEvaluator, toNumberArray()

### Community 101 - "epub/metadata.rs"
Cohesion: 0.19
Nodes (25): Attributes, BytesStart, HashMap, attribute(), calibre_series_fields_are_optional_and_tolerant(), detects_legacy_epub2_cover_meta(), EpubMetadata, handle_calibre_meta() (+17 more)

### Community 102 - "tts.js"
Cohesion: 0.12
Nodes (11): blockTags, fragmentToSSML(), getAlphabet(), getBlocks(), getFragmentWithMarks(), getLang(), getSegmenter(), ListIterator (+3 more)

### Community 103 - "assert"
Cohesion: 0.10
Nodes (4): assert(), CssFontInfo, SystemFontInfo, toRomanNumerals()

### Community 105 - "SimpleDOMNode"
Cohesion: 0.20
Nodes (3): isWhitespaceString(), enqueueChunk(), SimpleDOMNode

### Community 106 - "view.js"
Cohesion: 0.13
Nodes (15): textWalker(), fetchFile(), getFileEntries(), isCBZ(), isFB2(), isFBZ(), isPDF(), isZip() (+7 more)

### Community 107 - "KF8"
Cohesion: 0.13
Nodes (7): concatTypedArray(), concatTypedArray3(), getFragmentSelector(), KF8, parsePosURI(), parseResourceURI(), replaceSeries()

### Community 108 - "MessageHandler"
Cohesion: 0.13
Nodes (7): AbortException, MessageHandler, ResponseException, setVerbosityLevel(), UnknownErrorException, WorkerMessageHandler, wrapReason()

### Community 109 - "getPdfManager"
Cohesion: 0.15
Nodes (11): NetworkPdfManager, ensureNotTerminated(), finishWorkerTask(), getPdfManager(), loadDocument(), setupDoc(), onFailure(), onSuccess() (+3 more)

### Community 110 - "What You Must Do When Invoked"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 111 - "repository/collections.rs"
Cohesion: 0.16
Nodes (30): Collection, CollectionSummary, NewCollection, String, Utc, Vec, add_book_to_collection(), count_collections() (+22 more)

### Community 113 - "Documentation"
Cohesion: 0.08
Nodes (23): Archived Files, Basic Usage, Current Status, Demo, Documentation, EPUB CFI, foliate-js, Generating Images for Quotes (+15 more)

### Community 115 - "vendor/zip.js"
Cohesion: 0.09
Nodes (21): A, C, Ft, g(), h, I, j, k (+13 more)

### Community 116 - "search.rs"
Cohesion: 0.16
Nodes (20): build_fts_query(), empty_query_is_invalid_input(), finds_books_by_description(), finds_books_by_isbn_and_by_file_name(), finds_books_by_publisher(), finds_books_by_title(), match_syntax_is_never_injected_through_user_input(), multi_term_query_requires_every_term() (+12 more)

### Community 117 - "devDependencies"
Cohesion: 0.09
Nodes (23): devDependencies, eslint, @eslint/js, eslint-plugin-react-hooks, eslint-plugin-react-refresh, globals, jsdom, shadcn (+15 more)

### Community 120 - "JpegImage"
Cohesion: 0.20
Nodes (3): grayToRGBA(), JpegImage, resizeRgbImage()

### Community 121 - "ColorSpace"
Cohesion: 0.07
Nodes (6): AlternateCS, ColorSpace, DeviceGrayCS, DeviceRgbCS, isDefaultDecodeHelper(), PatternCS

### Community 122 - "compilerOptions"
Cohesion: 0.09
Nodes (22): compilerOptions, baseUrl, isolatedModules, jsx, lib, module, moduleDetection, moduleResolution (+14 more)

### Community 123 - ".bind"
Cohesion: 0.16
Nodes (4): FontLoader, isFontReady(), completeRequest(), string32()

### Community 124 - "4K scrolling performance — implementation plan"
Cohesion: 0.09
Nodes (21): 4K scrolling performance — implementation plan, Cross-cutting rules for every step, Dependencies, Phase 0 — Baseline + diagnostics (do first, ships with Phase 1), Phase 1 — Cap the render buffer (PDF-1 / PERF-1) — the primary fix, Phase 2 — Pixel-aware bitmap cache (PDF-2 / PERF-3), Phase 3 — Byte-budgeted live canvases + compositing hygiene (PDF-3 / PERF-4 + PERF-6), Phase 4 — Blit without realloc (PDF-3, small) (+13 more)

### Community 125 - "components.json"
Cohesion: 0.09
Nodes (21): aliases, components, hooks, lib, ui, utils, iconLibrary, menuAccent (+13 more)

### Community 126 - "FixedLayout"
Cohesion: 0.18
Nodes (4): FixedLayout, getViewport(), parseViewport(), TODO: index, overlayer

### Community 129 - "warn"
Cohesion: 0.11
Nodes (6): info(), makePathFromDrawOPS(), OptionalContentConfig, OptionalContentGroup, StatTimer, warn()

### Community 130 - "LabCS"
Cohesion: 0.11
Nodes (3): CalGrayCS, DeviceCmykCS, LabCS

### Community 131 - "dependencies"
Cohesion: 0.12
Nodes (17): dependencies, class-variance-authority, clsx, cn, @fontsource-variable/geist, lucide-react, pdfjs-dist, radix-ui (+9 more)

### Community 132 - "progressMigration.ts"
Cohesion: 0.17
Nodes (20): CfiStep, convertCfiStructure(), convertExact(), convertFoliateRow(), convertSpineElement(), convertSpineProgression(), cssPath(), elementChildren() (+12 more)

### Community 133 - ".wrap"
Cohesion: 0.15
Nodes (4): CFFCharset, CFFHeader, CFFStrings, Type1Font

### Community 134 - "EpubReader.test.tsx"
Cohesion: 0.07
Nodes (34): EpubRelocateDetail, EpubSearchCallbacks, BARE_MODIFIER_KEYS, comboFromEvent(), ShortcutContext, ShortcutHandler, ShortcutRegistry, ReaderProvider() (+26 more)

### Community 135 - ".constructor"
Cohesion: 0.13
Nodes (11): ae, b, be(), ie, je(), f(), me(), Q (+3 more)

### Community 136 - "fb2.js"
Cohesion: 0.16
Nodes (13): BODY, FB2Converter, getElementText(), makeFB2(), MIME, normalizeWhitespace(), NS, parseXML() (+5 more)

### Community 139 - ".fill"
Cohesion: 0.12
Nodes (6): convertBlackAndWhiteToRGBA(), convertRGBToRGBA(), convertToRGBA(), ImageResizer, PDFImage, resizeImageMask()

### Community 140 - "getTextRegionHuffmanTables"
Cohesion: 0.15
Nodes (11): decodeBlock(), decodeMcu(), decodeTablesSegment(), getBlockBufferOffset(), getCustomHuffmanTable(), getStandardTable(), getSymbolDictionaryHuffmanTables(), getTextRegionHuffmanTables() (+3 more)

### Community 141 - "commands/books.rs"
Cohesion: 0.26
Nodes (11): LibraryStats, get_library_stats(), list_books(), remove_book(), AppState, Book, Result, SearchHit (+3 more)

### Community 142 - "MOBI"
Cohesion: 0.14
Nodes (6): getString(), getVarLenFromEnd(), isMOBI(), MOBI, PDB, unescapeHTML()

### Community 143 - "OpenJPEG"
Cohesion: 0.16
Nodes (8): createWasm(), receiveInstance(), OpenJPEG(), assignWasmExports(), createWasm(), receiveInstance(), getWasmImports(), updateMemoryViews()

### Community 145 - "ChunkedStream"
Cohesion: 0.09
Nodes (4): arrayBuffersToBytes(), ChunkedStream, ChunkedStreamManager, MissingDataException

### Community 149 - ".getEntriesGenerator"
Cohesion: 0.20
Nodes (14): Bt(), Ct(), et, o(), It(), jt(), kt, Lt() (+6 more)

### Community 150 - "CFFCompiler"
Cohesion: 0.15
Nodes (3): CFFCompiler, CFFIndex, CFFOffsetTracker

### Community 152 - "FreeDrawOutliner"
Cohesion: 0.23
Nodes (3): FreeDrawOutliner, FreeHighlightOutline, FreeHighlightOutliner

### Community 154 - ".readBit"
Cohesion: 0.12
Nodes (17): ContextCache, decodeBitmap(), decodeBitmapTemplate0(), decodeHalftoneRegion(), decodeIAID(), decodeInteger(), readBits(), decodeMMRBitmap() (+9 more)

### Community 156 - "NewBook"
Cohesion: 0.14
Nodes (16): Book, book_serializes_with_camel_case_keys(), BookFormat, NewBook, Error, Ok, Option, Result (+8 more)

### Community 157 - "Stages"
Cohesion: 0.12
Nodes (15): Current state (verified), Decision points, Ground rules, Interaction model (binding), Plan: Bookshelf-inspired UI/UX architecture, Stage 0 — Foundations (complete), Stage 1 — Shell, navigation, shortcuts, Stage 2 — Library screen (+7 more)

### Community 158 - "4K scrolling performance — investigation handover"
Cohesion: 0.12
Nodes (15): 4K scrolling performance — investigation handover, Environment-1 — Silent slow paths in the GPU stack, EPUB-1 — Paginated turns rasterize viewport-sized CSS columns, EPUB-2 — Scrolled flow: whole-section iframe layers, Executive summary, External evidence, How to reproduce / measure, PDF-1 — No canvas resolution cap (primary) (+7 more)

### Community 159 - "MOBI6"
Cohesion: 0.15
Nodes (3): getIndent(), MOBI6, rawBytesToString()

### Community 160 - "paginator.js"
Cohesion: 0.16
Nodes (14): animate(), bisectNode(), easeOutQuad(), getBoundingClientRect(), getVisibleRange(), lerp(), makeRange(), TODO: it should probably allow elements that do not contain text (+6 more)

### Community 162 - "decodeScan"
Cohesion: 0.16
Nodes (20): buildComponentData(), decodeScan(), decodeACFirst(), decodeACSuccessive(), decodeBaseline(), decodeDCFirst(), decodeDCSuccessive(), decodeHuffman() (+12 more)

### Community 163 - "artwork_cache.rs"
Cohesion: 0.28
Nodes (15): cover_files(), make_covers_dir(), Option, Path, PathBuf, Result, SqlitePool, String (+7 more)

### Community 164 - "Rendering"
Cohesion: 0.13
Nodes (14): Behavior, Continuous reader architecture (`frontend/src/components/reader/pdf/`), Error handling, Import mapping, In-book search, Outline, PDF layer, Public API (+6 more)

### Community 166 - "getFilenameFromContentDispositionHeader"
Cohesion: 0.46
Nodes (8): getFilenameFromContentDispositionHeader(), fixupEncoding(), rfc2047decode(), rfc2231getparam(), rfc2616unquote(), rfc5987decode(), textdecode(), toParamRegExp()

### Community 167 - "ColorConverters"
Cohesion: 0.14
Nodes (5): ColorConverters, getModificationDate(), hexNumbers, makeColorComp(), scaleAndClamp()

### Community 170 - ".process"
Cohesion: 0.22
Nodes (6): addHex(), BinaryCMapReader, BinaryCMapStream, hexToInt(), hexToStr(), incHex()

### Community 172 - "Builder"
Cohesion: 0.15
Nodes (3): Builder, Empty, UnknownNamespace

### Community 173 - "ColorConverters"
Cohesion: 0.18
Nodes (3): ColorConverters, makeColorComp(), scaleAndClamp()

### Community 174 - "BasePDFStream"
Cohesion: 0.13
Nodes (4): BasePDFStream, BasePDFStreamRangeReader, PDFWorkerStream, PDFWorkerStreamRangeReader

### Community 175 - "services/reader.rs"
Cohesion: 0.31
Nodes (14): epub_book(), epub_error(), load_book_file(), load_book_file_range(), load_book_resource(), load_epub_session(), loads_the_stored_file_bytes_for_a_book_id(), missing_file_surfaces_as_an_io_error() (+6 more)

### Community 176 - "Tables"
Cohesion: 0.14
Nodes (14): annotations, book_collections, books, collections, Conventions, Database, Full-text search, library_locations (+6 more)

### Community 177 - "EPUB"
Cohesion: 0.16
Nodes (3): EPUB, getPageSpread(), resolveURL()

### Community 178 - "Loader"
Cohesion: 0.29
Nodes (3): Loader, pathRelative(), replaceSeries()

### Community 179 - "pdfRenderPolicy.test.ts"
Cohesion: 0.24
Nodes (10): MAX_ACTIVE_CANVAS_BYTES, MAX_RENDER_DIMENSION, MAX_RENDER_PIXELS, MAX_RENDER_PIXELS_HARD, RenderRatioOptions, CONTENT_1080P, CONTENT_4K, fitWidth() (+2 more)

### Community 180 - "bench-reader.e2e.ts"
Cohesion: 0.19
Nodes (8): appendTrend(), BenchReport, DragResult, FrameStats, percentile(), report, summarize(), writeReport()

### Community 181 - "compilerOptions"
Cohesion: 0.15
Nodes (12): compilerOptions, lib, module, moduleResolution, noEmit, noFallthroughCasesInSwitch, noUnusedLocals, skipLibCheck (+4 more)

### Community 183 - "CanvasExtraState"
Cohesion: 0.08
Nodes (8): applyBoundingBox(), BaseShadingPattern, CachedCanvases, CanvasExtraState, DummyShadingPattern, getShadingPattern(), MeshShadingPattern, RadialAxialShadingPattern

### Community 185 - "CFFDict"
Cohesion: 0.22
Nodes (3): CFFDict, CFFPrivateDict, CFFTopDict

### Community 186 - "AGENTS.md"
Cohesion: 0.17
Nodes (10): Branch state: stack migration, Commands (in this order), Conventions, E2E for agents, External Knowledge & Source Research, graphify, Keep this file compact, Non-obvious gotchas (+2 more)

### Community 187 - "Testing"
Cohesion: 0.17
Nodes (12): Benchmark suite (headed, opt-in), E2E (WebdriverIO against Electron), EPUB fixture corpus (three tiers), Frontend (Vitest + RTL), Headless on Linux (Xvfb), Isolation, cleanup, termination, Parallelism, Rules for agents (+4 more)

### Community 188 - "metadata.e2e.ts"
Cohesion: 0.26
Nodes (7): libraryDir, seededBookTitles, ensureLibrary(), openBookDetail(), returnToLibrary(), waitForLibraryView(), seededEpub

### Community 189 - "versions.ts"
Cohesion: 0.23
Nodes (11): repoRoot, assertDriverCompatibility(), connectedChromedriverVersion(), electronToChromium(), formatVersionBanner(), majorOf(), PkgVersion, requiredChromedriverVersion() (+3 more)

### Community 190 - ".constructor"
Cohesion: 0.24
Nodes (6): childGetter(), filterAttribute(), getHTMLFragment(), parseNav(), parseNCX(), Resources

### Community 191 - "footnotes.js"
Cohesion: 0.30
Nodes (9): extractFootnote(), FootnoteHandler, getReferencedType(), getRoles(), getTypes(), isFootnoteReference(), isSuper(), refRoles (+1 more)

### Community 193 - "Xe"
Cohesion: 0.17
Nodes (5): E(), ge, Ke, tt(), Xe

### Community 194 - "pdfOutline.ts"
Cohesion: 0.30
Nodes (5): convertItem(), isRecord(), normalizePdfOutline(), PdfOutlineSource, resolveDestPage()

### Community 195 - "make-fixture.py"
Cohesion: 0.24
Nodes (10): build_pdf(), main(), Path, 64x64 solid sky-blue PNG built without external dependencies., Deterministic multi-page PDF: catalog, page tree, Info dictionary, one page…, Generate tests/fixtures/books/minimal.epub and minimal.pdf — tiny valid book…, tiny_png(), uniform_pages() (+2 more)

### Community 196 - "Coding standards"
Cohesion: 0.18
Nodes (10): Coding standards, Comments, Commits, Dependencies, Do not over-engineer, Frontend, Keep it small, Rust (+2 more)

### Community 198 - "SectionProgress"
Cohesion: 0.24
Nodes (4): assignIDs(), flatten(), SectionProgress, TOCProgress

### Community 200 - "CalRGBCS"
Cohesion: 0.18
Nodes (3): CalRGBCS, IndexedCS, MathClamp()

### Community 201 - "XhtmlNamespace"
Cohesion: 0.18
Nodes (4): Li, Ol, Sup, XhtmlNamespace

### Community 202 - "y"
Cohesion: 0.22
Nodes (7): at(), le(), ne(), re(), se(), Wt(), y

### Community 203 - "commands/reader.rs"
Cohesion: 0.36
Nodes (10): get_book_bytes(), get_book_resource(), get_epub_session(), GetBookBytesResult, GetBookResourceResult, GetEpubSessionResult, AppState, Option (+2 more)

### Community 204 - "Reader rendering (Readium TS Toolkit)"
Cohesion: 0.20
Nodes (10): Annotations, Engine seam, In-book search, Position locator, Progress migration (foliate → Readium), Reader rendering (Readium TS Toolkit), Resource loading, Security (+2 more)

### Community 207 - "Datasets"
Cohesion: 0.20
Nodes (3): Datasets, datasets_Data, DatasetsNamespace

### Community 210 - "extended_epub.rs"
Cohesion: 0.33
Nodes (9): conformance_corpus_imports_every_book(), corpus_dir(), epub_files(), exercise_corpus(), extended_corpus_imports_every_book(), Option, Path, PathBuf (+1 more)

### Community 211 - "Architecture"
Cohesion: 0.22
Nodes (9): Architecture, Database layer, EPUB layer, Frontend structure, PDF layer, Process and boundary, Rust module contract, Services (+1 more)

### Community 212 - "Reader performance — bench findings and handover"
Cohesion: 0.22
Nodes (8): Candidate next levers (ordered), Executive summary, Open questions, Reader performance — bench findings and handover, Rules of engagement (unchanged), The benchmark suite (what exists and how to use it), The measured record (Kubuntu Wayland, AMD + Mesa 26.0.8, 4K panel @ 60 Hz), What is NOT the bottleneck (measured, don't re-chase)

### Community 213 - "watchdog.mjs"
Cohesion: 0.31
Nodes (8): killAll(), ownStart, parent, [parentPid, scratchDir, targetsRaw, display = ""], starttime(), sweep(), targets, timer

### Community 215 - "pdf.js"
Cohesion: 0.33
Nodes (7): makePDF(), makeTOCItem(), pdfjsPath(), TODO: for better results, parse `metadata.getRaw()`, TODO: this only works in Firefox; see…, render(), renderPage()

### Community 217 - "ue"
Cohesion: 0.28
Nodes (6): ce(), de(), fe(), he(), oe(), ue()

### Community 218 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 219 - "epub_corpus.rs"
Cohesion: 0.44
Nodes (8): core_corpus_stays_small(), corpus_root(), malformed_core_fixtures_are_rejected_for_both_generations(), malformed_fixtures(), PathBuf, Vec, valid_core_fixtures_parse_for_both_generations(), valid_fixtures()

### Community 221 - "Build and dev environment"
Cohesion: 0.25
Nodes (7): Build and dev environment, Chromedriver for E2E, Debug-build performance, Electron + Vite, Legacy (pre-migration, until phase 5), PDFium shared library (PDF covers), Rust sidecar build

### Community 222 - "Electron migration plan"
Cohesion: 0.25
Nodes (8): Electron migration plan, Phase 1 decisions and state (2026-09-07), Reading-progress migration (user data, must survive), Removal checklist (phase 5), Resource protocol, Rust service (sidecar), Status, Target process model

### Community 223 - "Performance budgets and metrics"
Cohesion: 0.25
Nodes (7): Budgets, How to measure, Legacy notes (WebKitGTK era, kept for context), Performance budgets and metrics, Reference conditions, Rules for changes, Touch-list

### Community 224 - "fetch-chromedriver.mjs"
Cohesion: 0.32
Nodes (6): cacheDir, downloadChromedriver(), electronVersion(), here, installedBinary(), repoRoot

### Community 225 - "scripts"
Cohesion: 0.25
Nodes (8): scripts, build, dev, lint, preview, test, test:ci, typecheck

### Community 226 - "getStruct"
Cohesion: 0.36
Nodes (6): getEXTH(), getFont(), getStruct(), getUint(), huffcdic(), read32Bits()

### Community 227 - ".#turnPage"
Cohesion: 0.25
Nodes (4): debounce(), selectionIsBackward(), setSelectionTo(), wait()

### Community 231 - "TuxBooks"
Cohesion: 0.25
Nodes (8): Commands, Documentation, Getting started, Install, Layout, License, Stack, TuxBooks

### Community 232 - "ROADMAP.md"
Cohesion: 0.25
Nodes (7): Completed foundation, Current State, Milestone Completion Standard, Product Vision, Purpose, Recommended Execution Order, TuxBooks Milestones

### Community 233 - "Development Principles"
Cohesion: 0.25
Nodes (8): Avoid premature generalization, Development Principles, Local-first, No fake persistence, One subsystem at a time, Preserve working architecture, Real E2E matters, Tests are part of the feature

### Community 234 - "Milestone 6 — Bookmarks, Highlights, and Notes"
Cohesion: 0.25
Nodes (8): Bookmarks, Data model, Exit criteria, Goal, Highlights, Milestone 6 — Bookmarks, Highlights, and Notes, Notes, Reader UI

### Community 235 - "Milestone 4 — Covers and Artwork Pipeline"
Cohesion: 0.25
Nodes (8): Cache, EPUB, Exit criteria, Goal, Milestone 4 — Covers and Artwork Pipeline, PDF, Requirements, Testing

### Community 236 - "Scope"
Cohesion: 0.25
Nodes (8): EPUB engine, EPUB locator, EPUB reader, Exit criteria, Goal, Milestone 1 — Production EPUB Reader, Scope, Testing

### Community 237 - "Milestone 2 — PDF Navigation"
Cohesion: 0.25
Nodes (8): Exit criteria, Goal, Milestone 2 — PDF Navigation, Page thumbnails, PDF outline, Reader navigation, Scope, Testing

### Community 238 - "fetch-epub-extended.py"
Cohesion: 0.50
Nodes (7): already_cached(), extract(), fetch(), main(), Path, Fetch opt-in extended/conformance EPUB datasets (Tiers B and C). This is the…, sha256_file()

### Community 239 - "EPUB layer"
Cohesion: 0.29
Nodes (7): EPUB layer, Error handling, Fixtures, Known limitations (intentional, for now), Parsing stages, Public API, Rust import parser

### Community 240 - "Release and distribution"
Cohesion: 0.29
Nodes (6): AppImage specifics, Artifacts, Cutting a release, Deliberate deferrals, Release and distribution, The packaging gate

### Community 241 - "Webview frame clock and dpr fiction — findings"
Cohesion: 0.29
Nodes (6): Appendix — probe sources, Verdict 1 — the ~31 fps idle ceiling is WebKitGTK engine-level, Verdict 2 — dpr 2 is fiction; the true compositor scale is 1.45, Webview frame clock and dpr fiction — findings, What this changes in the handover's lever list, Why no clamp was implemented (decision)

### Community 242 - "search.js"
Cohesion: 0.57
Nodes (6): makeExcerpt(), normalizeWhitespace(), search(), searchMatcher(), segmenterSearch(), simpleSearch()

### Community 244 - "books"
Cohesion: 0.21
Nodes (12): book_collections, books, collections, reading_progress, annotations, authors, book_authors, book_metadata_overrides (+4 more)

### Community 245 - "DeviceRgbaCS"
Cohesion: 0.29
Nodes (3): copyRgbaImage(), DeviceRgbaCS, resizeRgbaImage()

### Community 247 - "Milestone 10 — Library and Reader UX Polish"
Cohesion: 0.29
Nodes (7): Accessibility, Desktop behavior, Exit criteria, Goal, Library, Milestone 10 — Library and Reader UX Polish, Reader

### Community 248 - "Milestone 7 — Metadata and Library Curation"
Cohesion: 0.29
Nodes (7): Architecture, Exit criteria, Goal, Metadata, Milestone 7 — Metadata and Library Curation, Normalized entities, Optional future operation

### Community 249 - "Milestone 3 — Filesystem Watcher and Library Reconciliation"
Cohesion: 0.29
Nodes (7): Exit criteria, Goal, Milestone 3 — Filesystem Watcher and Library Reconciliation, Missing files, Reconnection, Scope, Testing

### Community 250 - "BookMetadata"
Cohesion: 0.48
Nodes (6): BookMetadata, MetadataFields, MetadataOverridden, Option, String, Vec

### Community 251 - "realistic_library_imports_every_book_file"
Cohesion: 0.43
Nodes (6): count_book_files(), realistic_library(), realistic_library_imports_every_book_file(), Option, Path, PathBuf

### Community 256 - "ve"
Cohesion: 0.33
Nodes (3): te(), ve, p()

### Community 257 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 258 - "Milestone 8 — Unified Reader Model"
Cohesion: 0.33
Nodes (6): Architecture, Document-specific position, Exit criteria, Goal, Milestone 8 — Unified Reader Model, Shared concepts

### Community 259 - "Milestone 9 — Reader Reliability and Performance Hardening"
Cohesion: 0.33
Nodes (6): Async lifecycle, Exit criteria, Goal, Memory, Milestone 9 — Reader Reliability and Performance Hardening, Test interactions

### Community 260 - "Milestone 11 — Release and Distribution"
Cohesion: 0.33
Nodes (6): CI, Exit criteria, Goal, Milestone 11 — Release and Distribution, Scope, Versioning policy (pre-1.0)

### Community 261 - "Milestone 5 — Search"
Cohesion: 0.33
Nodes (6): EPUB in-book search, Exit criteria, Goal, Library search, Milestone 5 — Search, PDF in-book search

### Community 262 - "default.json"
Cohesion: 0.33
Nodes (5): description, identifier, permissions, $schema, windows

### Community 263 - "ReadingProgress"
Cohesion: 0.47
Nodes (5): ProgressUpdate, ReadingProgress, Option, String, Utc

### Community 264 - "foliate-js.d.ts"
Cohesion: 0.40
Nodes (3): @/lib/epub/foliate-js/overlayer.js, @/lib/epub/foliate-js/view.js, Overlayer

### Community 267 - "quote-image.js"
Cohesion: 0.50
Nodes (3): fit(), getBlob(), TODO: lang, vertical writing

### Community 274 - ".prettierrc.json"
Cohesion: 0.40
Nodes (4): printWidth, semi, singleQuote, trailingComma

### Community 275 - "Button"
Cohesion: 0.22
Nodes (4): A, Button, fixURL(), recoverJsURL()

### Community 276 - "Coverage gate"
Cohesion: 0.50
Nodes (3): Coverage gate, Outside the gate (and why), Required coverage by category

### Community 277 - "Confirming comment for WebKit bug 315997"
Cohesion: 0.50
Nodes (3): Comment text (copy everything inside the fence), Confirming comment for WebKit bug 315997, Follow-up comment — version A/B (paste only after the first comment)

### Community 282 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 283 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 284 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 285 - "coverage-gate.mjs"
Cohesion: 0.50
Nodes (3): byModule, cov, MODULES

### Community 316 - "library_locations.rs"
Cohesion: 0.36
Nodes (6): add_location(), list_locations(), Result, SqlitePool, String, Vec

## Knowledge Gaps
- **1031 isolated node(s):** `$schema`, `plugin`, `printWidth`, `semi`, `singleQuote` (+1026 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 2847 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **96 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `TemplateNamespace` connect `XFAObject` to `pdf.worker.mjs`, `getStringOption`, `ConfigNamespace`, `find`, `Dict`, `ExclGroup`, `StringObject`, `Area`, `AnnotationEditor`, `.fill`, `getPdfManager`, `.success`, `Button`, `ContentObject`, `Subform`?**
  _High betweenness centrality (0.137) - this node is a cross-community bridge._
- **Why does `Debug` connect `library_watcher.rs` to `pdf.worker.mjs`, `XFAObject`?**
  _High betweenness centrality (0.125) - this node is a cross-community bridge._
- **Why does `LibraryWatcher` connect `library_watcher.rs` to `EventEmitter`?**
  _High betweenness centrality (0.124) - this node is a cross-community bridge._
- **Are the 2 inferred relationships involving `warn()` (e.g. with `.decode()` and `.decode()`) actually correct?**
  _`warn()` has 2 INFERRED edges - model-reasoned connections that need verification._
- **What connects `$schema`, `plugin`, `printWidth` to the rest of the system?**
  _1031 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `pdf.worker.mjs` be split into smaller, more focused modules?**
  _Cohesion score 0.006865309992449915 - nodes in this community are weakly interconnected._
- **Should `getStringOption` be split into smaller, more focused modules?**
  _Cohesion score 0.021992358587270524 - nodes in this community are weakly interconnected._
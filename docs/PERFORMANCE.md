...
| PERF-2  | Visible-page raster latency (render start → blit)           | p95 ≤ 100 ms at reference conditions                                                                                                                                                                                                                                      | Re-baselined on Chromium/MuPDF (2026-09-09, phase 6): p50 147.5 ms / p95 176.5 ms, maximized 2643×1405 at dpr 1.45 (closest achievable here; bench has no dpr override). MuPDF worker raster dominates at this viewport; exceeds the WebKitGTK-era threshold — enforce at true reference conditions (3840×2160, dpr 1/2) before treating 100 ms as a gate                                                                                                                                                                                                                                                               | Manual measurement (`just bench-reader`); diagnostic attrs                                          |
...
| PERF-14 | PDF open path — first useful page                           | The first readable page must not wait for the whole file: documents open through the range-backed `mupdf.Stream` (`openPdfDocumentFromBook`); engine prewarm skips WASM cold start; outline/text/thumbnails are ordered below first paint                                 | Range-backed open + engine prewarm landed (2026-09-09); click→open / firstPaint / interactive segments recorded on the reader element. Timing thresholds are manual-bench values, never CI gates                                                                                                                                                                                                                                                                                                                                                                                                                        | `data-pdf-open-state` / `data-pdf-open-timing` attrs (E2E asserts state only) + `just bench-reader` |
...
| PERF-17 | PDF deep-zoom raster policy                                 | Above the whole-page budget, rasterize only the visible region at device resolution — no page-sized buffer, no CSS upscale of sub-display-resolution pixels                                                                                                               | Viewport-clipped region rendering: `needsRegionRender`/`regionRenderRatio` (`pdfRenderPolicy`), `visiblePageRegion` (`pdfLayout`), a region-sized, absolutely positioned canvas in `PdfPageCanvas`, and a clip in the `mupdfWorker` render. Measured (100-page fixture, Smart Dark, 10000%): largest page buffer 33.5 MP → 3.6 MP, max main-thread frame gap 283 ms → 50 ms. Fit-width path unchanged. Superseding rasters coalesce (150 ms settle, first paint immediate) so rapid zoom cannot queue worker renders. Worker raster at deep zoom remains content-bound (~1.6 s/page here); content culling is follow-up | Unit (region math, policy, cache key) + E2E `data-pdf-render-region` geometry                       |
